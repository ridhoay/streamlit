...
def search():
    ...
    query = "SELECT * FROM jobs WHERE title LIKE ? AND location LIKE ? AND type LIKE ? AND company LIKE ?"
    search_results = query_db(query, (f"%{name}%", f"%{location}%", f"%{job_type}%", f"%{company}%"))
...
def registration_applicant():
    ...
    if request.method == "POST":
        ...
        query_db("INSERT INTO users (username, hash, user_type) VALUES (?, ?, ?)", (username, hashed_password, user_type))
...
def create_job():
    ...
    if request.method == "POST":
        ...
        query_db("INSERT INTO jobs (title, company, type, location, description, status, posted_by) VALUES (?, ?, ?, ?, ?, ?, ?)",
                 (job_title, company_name, job_type, job_location, job_description, job_status, posted_by))
...
def manage_job():
    ...
    posted_jobs = query_db("""SELECT id, title, company, type, location, description, status, applicants
                                FROM jobs
...
def edit_job(job_id):
    ...
    if job and job["posted_by"] == username:
        if request.method == "POST":
            ...
            query_db("""UPDATE jobs
                          SET title = ?, 
                              ...
                              status = ?
                          WHERE id = ?""",
                       (updated_job_title, updated_company_name, updated_job_type, updated_job_location,
                       updated_job_description, updated_job_status, job_id))
...
def all_jobs():
    if request.method == "GET":
        posted_jobs = query_db("""SELECT * FROM jobs
                                    WHERE status = "active";""")
...
def job_details(job_id):
    ...
    if request.method == "POST":
        ...
        previous_application = query_db(
            "SELECT id FROM applied_jobs WHERE applicant = ? AND job_applied = ?",
            (user_id, job_id))
        ...
        query_db(
            "INSERT INTO applied_jobs (applicant, job_applied, apply_time) VALUES (?, ?, ?)",
            (user_id, job_id, application_time))
        query_db("UPDATE jobs SET applicants = applicants + 1 WHERE id = ?", [job_id])
        flash("Application successful!", "success")
        return redirect('/dashboard')

    return render_template("job-details.html", job=job, username=username)
...
def manage_application():
    ...
    applied_jobs = query_db("""SELECT jobs.*, applied_jobs.apply_time
                                    FROM jobs
...
def withdraw_application():
    ...
    if applied_jobs:
        ...
        query_db("DELETE FROM applied_jobs WHERE id = ?", [application_id])
        query_db("UPDATE jobs SET applicants = applicants - 1 WHERE id = ?", [job_id])
